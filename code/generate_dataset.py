...
class CubDADataset(data.Dataset):
    ...
    def load_da_data(self):
        self.da_filenames = {}
        self.da_captions = {}
        # self.da_classid = {}

        # with open('/data3/private/mxy/projects/mmda/data/birds/captions.pickle', 'rb') as f:
        #     x = pickle.load(f)
        #     self.ixtoword, self.wordtoix = x[2], x[3]
        #     del x

        with open('{}/sample_captions.txt'.format(self.da_root), 'r') as f:
            lines = f.read().splitlines()
            all_captions = {}
            self.filtered_names = None
            filter_path = '%s/filtered_names-80k.txt'%(self.da_root)
            if os.path.isfile(filter_path):
                with open(filter_path) as f:
                    self.filtered_names = [l.split(':\t')[0].replace('.jpg', '') for l in f.read().splitlines()]
            for l in tqdm(lines, desc='DA Captions'):
                s = l.split(':\t')
                if self.filtered_names is not None and s[0] not in self.filtered_names:
                    continue
                # filepath = s[0].split('/')
                # self.da_filenames.append(filepath[-1])
                filepath = s[0]
                key = filepath.split('-')[0]
                if key in self.da_filenames:
                    self.da_filenames[key].append(filepath)
                else:
                    self.da_filenames[key] = [filepath]

                # self.da_classid[filepath] = int(filepath[0].split('.')[0])
                caption = s[1].replace(' [SEP]', '').replace('[CLS] ', '').strip()
                ...
                for t in tokens:
                    t = t.encode('ascii', 'ignore').decode('ascii')
                    if t not in self.wordtoix:
                        self.ixtoword[len(self.ixtoword)] = t
                        self.wordtoix[t] = len(self.wordtoix)
                        with open('wrong_tokens.txt', 'a') as fout:
                            fout.write(str(t)+'\t'+filepath+'\n')
                    if len(t) > 0:
                        tokens_new.append(t)
                if len(tokens_new) == 0:
                    import ipdb; ipdb.set_trace()
                self.da_captions[filepath] = [self.wordtoix[t] for t in tokens_new]

        self.da_keys = sorted(list(self.da_filenames.keys()))

    def get_caption(self, filename):
        # a list of indices for a sentence
        sent_caption = np.asarray(self.da_captions[filename]).astype('int64')
        if (sent_caption == 0).sum() > 0:
            print('ERROR: do not need END (0) token', sent_caption)
        num_words = len(sent_caption)
        # pad with 0s (i.e., '<end>')
    ...
    def __getitem__(self, index):
        # key = self.da_keys[index//10]
        # filepath = self.da_filenames[key][index%10]

        key = self.da_keys[index]
        filepath = random.choice(self.da_filenames[key])

        cls_id = int(key.split('.')[0])
        # cls_id = index
        img_name = '%s/%s' % (self.da_root, filepath)
        ...
        caps, cap_len = self.get_caption(filepath)
        
        return imgs, caps, cap_len, cls_id, key

    def __len__(self):
        # return len(self.da_keys) * 10
        return len(self.da_keys)
...
class ConcatDataset(data.Dataset):
    def __init__(self, datasets):
        ...
        self.ixtoword = datasets[0].ixtoword
        self.n_words = datasets[0].n_words
        self.wordtoix = datasets[0].wordtoix
    ...
    def get_mis_caption(self, cls_id):
        return self.datasets[0].get_mis_caption(cls_id)
